...
description: "Always check the HTTP response code to correctly handle success and error conditions from the Giftbit API."
---
...
<Warning>
  **Only** an HTTP status of `200` means the request was successful. For non-`200` responses, the predictable response body described below gives details on why the request failed. We recommend logging any failure response body as best practice — the Giftbit support team needs the full response body to assist with troubleshooting.
</Warning>

## Success responses

**`200`** — Success. See the [API reference](/api-reference/ping/ping) for the success response format of each endpoint.

## Error responses

All non-`200` responses should be treated as an error and either retried, or the request fixed and then retried. To assist with programmatic error handling and troubleshooting, all error response bodies are returned in the following JSON format:
...
<AccordionGroup>
  <Accordion title="400 — Malformed request">
    Error due to a malformed request body, such as broken JSON or a missing required element. Requires fixing up the request body.
  </Accordion>
  ...
  <Accordion title="429 — Too many requests">
    ...
    You can simulate this condition in the Testbed environment — without actually producing a high request rate — by passing a `SIMULATE-RATELIMIT` header with a value of `true` on any request. If you feel you are being frequently rate limited during valid usage, please contact support.
  </Accordion>
  <Accordion title="5xx — Unexpected Giftbit error">
